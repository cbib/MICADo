...
class ReferenceGraph:
	def __init__(self, kmer_length, fasta_file, snp_file):
		...
		for l in range(1, len(lines), +2):
			...
			self.snp[line_before_split[0]] = [line_split[0][len(line_split[0]) - kmer_length + 1:len(line_split[0])] + line_split[1].split("|")[0],
											  line_split[0][len(line_split[0]) - kmer_length + 1:len(line_split[0])] + line_split[1].split("|")[1]]
